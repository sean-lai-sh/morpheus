...
- After seed, `/meet` picker and `@mention` attendees resolve by snowflake only. Detect mapped roster roles by snowflake (`<@&1203562091500404782>` / picker value), not the word “eboard”. That role means the F26 Preferred Email dump plus any extra explicit user snowflakes — not live Discord role expansion.
