...
- Booking a Calendar meeting is **explicit**: the `/meet` slash command is the only door. @-mentioning the bot never books a meeting — mention traffic falls through to the ordinary job enqueue path unless it is a high-confidence NL todo (`add a todo … by friday 2pm`, `what's on my list`, `mark … done`). Those reuse the `/task` tables, require a due date, and never invent a time.
- NL todo reminders fire 1 day and 5 hours before due: channel `@` the assignee in `tasks.channel_id` plus the existing DM. Channel delivery is a `channel_reminder` pin on the assignment, not the reminder policy name. `/task` slash policies stay DM-only and cannot select `one_day_and_five_hours`. Changing a personal reminder override clears the pin.
- Elapsed reminder slots are skipped, so a todo created inside the window never posts a ping labelled "1-day reminder" the moment it is made. A todo due in under five hours reminds once, now, as the 5-hour slot.
- Completion needs a target: a bare `done` lists your open todos and asks which, it never picks one for you. An @mention that merely starts with "complete …" is agent work and falls through to the job queue.
- An NL todo assigns at most 25 people; a role mention past that is refused rather than expanded into a mass ping. Use `/task` for a larger group.
- Author-bound `/v1/tasks` is for claimed jobs only (SDK/Grok fallthrough). The workspace bearer is not a user identity.
